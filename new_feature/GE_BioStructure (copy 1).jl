...
function main()
    ...
    # incase end of index does not specified, run for end of list
    if end_idx == 0
        end_idx = length(pdb_files)
    end

    output_filename = string("MAX_GC_", start_idx, "_", end_idx, ".dat")
    ...
    for idx_file in start_idx:end_idx
        file = string(base_path, "/", pdb_files[idx_file], "_v4.pdb")
        coor = read_coordinates(file)
        i1, i2, j1, j2, maxG = GLN(coor)
        @printf("%d, %s, [(%d, %d) | (%d, %d)], %.3f\n", idx_file, pdb_files[idx_file], i1, i2, j1, j2, maxG)
